...
function Noise({
  ...
  patternAlpha?: number
  intensity?: number
  postDraw?: (ctx: CanvasRenderingContext2D, frame: number, timestamp: number, cssW: number, cssH: number) => void
}) {
  const grainRef = useRef<HTMLCanvasElement>(null)
  const canvasCssSizeRef = useRef({ width: 0, height: 0 })
  const postDrawRef = useRef(postDraw)
  postDrawRef.current = postDraw
  ...
  useEffect(() => {
    ...
    const loop = (timestamp: number) => {
      if (canvasCssSizeRef.current.width > 0 && canvasCssSizeRef.current.height > 0) {
        ...
        if (patternInitialized && postDrawRef.current) {
          const { width: w, height: h } = canvasCssSizeRef.current
          postDrawRef.current(ctx, frame, timestamp, w, h)
        }
      }
      frame++
